...
/**
 * เลือกรูปปกให้บทความ
 * ลำดับการหา: หมวดที่ระบุ → หมวดรวม (all) → หมวดสำรอง → รูปเดิม
 */
export function pickCover(theme, slug) {
  const order = [theme, "all", DEFAULT_THEME].filter(Boolean);
  for (const t of order) {
    const files = listPool(t);
    if (files.length > 0) {
      const idx = hashSlug(slug) % files.length;
      return `/images/covers-pool/${t}/${files[idx]}`;
    }
  }
...
function normalize(raw) {
  ...
  return {
    ...
    ...raw,
    coverTheme: theme,
    fallbackImage: pickCover(theme, raw.slug),
    dateIso,
